...
export default function Notifications() {
  return (
    <>
      <div className="mb-8">
        <h3 className="text-sm font-semibold text-gray-900 dark:bg-black/80 dark:text-white mb-4">
          Notifications
        </h3>
          ...
          {NOTIFICATIONS.map((item) => (
            <div key={item.id} className="flex gap-3 group">
              ...
              <div className="flex flex-col">
                <span
                  className="text-sm text-gray-800 dark:bg-black/80 dark:text-white line-clamp-1"
                  title={item.message}
                >
                  {item.message}
                </span>
